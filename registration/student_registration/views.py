    ...
    with transaction.atomic():
        try:
            ...
            if student.is_previous_student:
                ...
                student.current_islamic_grade = student.grade_in_school
            else:
...
def searchforattendance(request):
    try:
        ...
        students = StudentDetails.objects.filter(
            current_islamic_grade=int(request_data['grade']),
            status='approved',
...
def search(request):
    """
        ...
        with Grade = 1 for the column current_islamic_grade. Lets presume you get 3 students.
